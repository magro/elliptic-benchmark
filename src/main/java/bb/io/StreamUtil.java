...
public final class StreamUtil {
	...
	*/
	private static final int bufferSizeDrainMin = 8 * 1024;
// +++*** concerning buffer size and I/O read performance--look at my BenchmarkFileWriting class wherein I found that 8 KiB buffers are typically optimal, and going larger is worthless
// this means that I need to redo all the buffering code below and in other classes...
	...
	* since otherwise drain cannot work.
		...
		try {
			...
			if (worker.isAlive()) {
				...
				try {
					...
					Logger.getAnonymousLogger().logp(Level.WARNING, "StreamUtil", "drain", "encountered a problem interrupting the temporary worker thread", t);
				}
				...
				try {
					...
					Logger.getAnonymousLogger().logp(Level.WARNING, "StreamUtil", "drain", "encountered a problem waiting for the temporary worker thread to die", t);
				}
				
					// if still alive best can do is minimize its damage by deprioritizing it:
				try {
					if (worker.isAlive()) {
						Logger.getAnonymousLogger().logp(Level.WARNING, "StreamUtil", "drain", "the temporary worker thread does not appear to have died");
						worker.setPriority( Thread.MIN_PRIORITY );
					}
				}
				catch (Throwable t) {	// HAVE seen setPriority throw a NullPointerException when it is in the process of being shutdown
					Logger.getAnonymousLogger().logp(Level.WARNING, "StreamUtil", "drain", "encountered a problem when trying to deprioritize the temporary worker thread", t);
				}
	...
	*/
	...
	// -------------------- getLines, getLineTokens --------------------
	
/*
+++*** finish this code

if want really high performance, should return the SAME CharSequence instance and require user to do new String if want to retain data

	*
	* Returns a new Iterator which iterates thru all the lines that can be read from reader.
	* This allows reader to be conveniently treated as if it were a Collection of lines instead of a stream.
	* <p>
	...
	motivation: much more efficient than reading in entire contents of, say a large file
	very efficient for parsing of simple file formats such as csv
	* <p>
	* @throws IllegalArgumentException if reader == null
	*
	public static Iterator<CharSequence> getLines(Reader reader) throws IllegalArgumentException, EOFException, IOException {
	...
	private static class LineIterator implements Iterator<CharSequence> {
		...
		private CharSequence lineNext;
		
		private LineTokenIterator(Reader reader) {
			br = (reader instanceof BufferedReader) ? (BufferedReader) reader : new BufferedReader(reader);
		}
		
		public boolean hasNext() {
			readLineNextIfNull();
			return (lineNext != null);
		}
		
		public CharSequence next() throws NoSuchElementException {
			readLineNextIfNull();
			if (lineNext == null) throw new NoSuchElementException("end of stream (readLine returned null)");
			CharSequence result = lineNext;
			lineNext = null;        // CRITICAL: must null lineNext to mark it as having been iterated over
			return result;
		}
		...
		private void readLineNextIfNull() {
			if (lineNext == null) {
				ineNext = br.readLine();
			}
		}
	...
	**
	* Returns a new Iterator which will iterate thru all the lines that can be read from reader.
	* This allows reader to be conveniently treated as if it were a Collection of lines instead of a stream.
	* <p>
	
	delimiter is a literal char
		does not support any kind of escaping, so beware using it as a generic csv parser
		typically is a single char, like a comma (',') or a tab ('\t')
		
	on why I did not use regex:
	csv intro:
	...
	csv regex:
		...
		http://www.thatsjava.com/java-essentials/41532/
	csv IN GENERAL is impossible to parse with regex because of all the special cases imposed by allowing commas, quotes, newlines to be inside field values:
		http://www.coderanch.com/t/379936/java/java/Regular-Expression-Parse-CSV
		
	headerTokensExpected may be null
	
	does not close in, that is the responsibility of the caller
	
	* <p>
	* @throws IllegalArgumentException if reader == null
	* @throws IllegalStateException if headerTokensExpected != null and the first line's tokens do not equal it
	
	* @throws EOFException if Hit End Of Stream before reading all the requested chars
	* @throws IOException if an I/O problem occurs
	*
	public static Iterator<CharSequence[]> getLineTokens(Reader reader, char delimiter, String... headerTokensExpected) throws IllegalArgumentException, EOFException, IOException {
		Check.arg().notNull(reader);
		
		LineIterator lineIterator = new LineIterator(reader);
		String delimiterRegex = Pattern.quote( String.valueOf(delimiterChar) );
		
		if (headerTokensExpected != null) {
			if (lineIterator.hasNext()) {
					CharSequence lineFirst = lineIterator.next();
					String[] headerTokens = lineFirst.split(delimiterRegex, -1);
					if (!Arrays.equals(headerTokens, headerTokensExpected)) throw new IllegalStateException("headerTokens !equals headerTokensExpected; details:" + "\n" + "headerTokens (tab delimited) = " + StringUtil.toString(headerTokens, "\t") + "\n" + "headerTokensExpected (tab delimited) = " + StringUtil.toString(headerTokensExpected, "\t") );
			}
		}
		
		return new LineTokenIterator(lineIterator, delimiterRegex);
	}
	
	private static class LineTokenIterator implements Iterator<CharSequence[]> {
		
		private final LineIterator lineIterator;
		private final String delimiterRegex;
		
		private LineTokenIterator(LineIterator lineIterator, String delimiterRegex) {
		...
		public CharSequence[] next() throws NoSuchElementException {
			if (!hasNext()) throw new NoSuchElementException("end of stream (readLine returned null)");
			
			return lineIterator.next().split(delimiterRegex, -1);
		}
	...
	*/
	...
	private static class TransferProgressReporter {
		...
		private void update(int bytesTransferred, int available) {
			...
			logger.println(getTimeStamp( new Date(now) ) + "\t" + totalTransferred + "\t" + available + "\t" + rateOverall + "\t" + rateLast);
			last = now;
		}
	...
	*/
	public static void close(Closeable closeable) {
		try {
			...
			Logger.getAnonymousLogger().logp(Level.SEVERE, "StreamUtil", "close", "caught an unexpected Throwable", t);
		}
	}
...
// +++ should I add varargs versions of these methods, so that can supply all the resources in a mthod that need closing in a single call?
//	--main issue: performance may go slightly down, since will be implicitly creating an Object[]; on the other hand, this may be nothing compared to the costs of using the resource...
