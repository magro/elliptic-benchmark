...
*/
	...
	/**
	* Determines whether or not c is "blank" (i.e. a normally invisible character).
	* The result is true if c is a {@link Character#isWhitespace whitespace}, {@link Character#isISOControl control}, or {@link Character#FORMAT format} char,
	* and is false otherwise.
	*/
	public static boolean isBlank(int c) {
		if ((33 <= c) && (c <= 126)) return false;	// CRITICAL performance optimization: if assume that most values of c are non-blank ascii chars, then this test will work and is much faster than calling the code below
	...
	*/
	...
	public static class UnitTest {
		...
		@Test public void test_isBlank() {
				// go thru all the ASCII chars:
			for (int i = 0; i <= 32; i++) {
				Assert.assertTrue( isBlank(i) );
			}
			for (int i = 33; i <= 126; i++) {
				Assert.assertFalse( isBlank(i) );
			}
			Assert.assertTrue( isBlank(127) );
		}
