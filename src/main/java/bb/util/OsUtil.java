	...
	public static void execAsynch(String command, String[] envp, File dir, StreamDrainer outDrainer, StreamDrainer errDrainer) throws IllegalArgumentException {
		...
		Runnable backgroundTask = new Runnable() { public void run() {
			try {
				...
				Logger.getAnonymousLogger().logp(Level.INFO, "OsUtil", "execAsynch", "Execution succeeded:" + "\n" + task.getState());
			}
			catch (Throwable t) {
				Logger.getAnonymousLogger().logp(Level.SEVERE, "OsUtil", "execAsynch", "Execution FAILED", t);	// Note that t should normally be the RuntimeException thrown by task.run, and so it should have the details from task.getState already embedded in it
			}
		} };
