...
* <p>
* Note that several methods in this class (e.g. {@link #getMethod})
* involve <code>Class[]</code> instances (e.g. to specify method signatures).
* For a method argument which is some <code>Object</code> type,
* the <code>Class</code> is easily specified either dynamicly (e.g. <code>obj.getClass()</code>)
* or staticly (e.g. <code>String.class</code>).
* For a method argument which is some primitive type,
* the <code>Class</code> can only specified staticly (e.g. <code>double.class</code> or <code>{@link Double#TYPE}</code>).
* See <a href="http://download.oracle.com/javase/tutorial/reflect/class/classNew.html">this discussion</a>
* or <a href="http://stackoverflow.com/questions/180097/dynamically-find-the-class-that-represents-a-primitive-java-type">this discussion</a>.
* <p>
	...
	*/
	public static Object callLogError(Object obj, String methodName, Class[] paramClasses, Object[] paramValues) {
		try {
			...
			Logger.getAnonymousLogger().logp(Level.SEVERE, "ReflectUtil", "callLogError", "caught an unexpected Throwable", t);
			return null;
		}
	}
