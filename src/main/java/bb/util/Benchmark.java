...
+++ WARNING: BENCHMARKING MULTIPLE THINGS IN A SINGLE JVM SESSION IS BAD: PREVIOUS JVM HISTORY CAN AFFECT THE RESULTS
	...
	--work arounds:
		1) have each JVM session do a single benchmark
			...
			--cons: shell script languages, especially dos bat files, can be nasty
			--NOTE: the next +++ discusiion below mentions another reason why should do this technique
...
+++ BE AWARE THAT SOMETIMES YOU GET SIGNIFICANTLY DIFFERENT BENCHMARK RESULTS FROM JVM RUN TO JVM RUN
	...
	--Josh attributes this to random inlining judgements being made by hotspot, with one set of inlining being much better than another; is non-deterministic because multi threads are involved
		--the keyword seems to be "compilation planning"
	--Cliff Click also goes over this phenomena in pp. 11-13 of this 2009 JavaOne talk
		http://developers.sun.com/learning/javaoneonline/sessions/2009/pdf/TS-5391.pdf
	--Cliff recommends doing up to 30 separate JVM benchmark runs to get statistically significant results

--another thing Cliff Click mentions starting on p. 14 of this 2009 JavaOne talk
	http://developers.sun.com/learning/javaoneonline/sessions/2009/pdf/TS-5391.pdf
is his advice that "Timing harness needs to invoke many targets".
By this, he means that you should execute 2 different classes which both implement the same interface (e.g. Runnable) and which both internally have a copy of the code to be benchmarked.
He advises this for two reasons:
	1) DCE
	2) to get consistent measurement times in view of inlining randomness (by having 2 different targets, the JVM is forced to deoptimize virtual calls, so you are ensured that no inlining takes place at the Runnable.run level)
I had a long email discussion with him about this; see my file
	cliffClickEmailsOnBenchmarking.tx	
My conclusion is that this is not a good thing to do:
	1) your benchmark code is vulnerable to DCE on many levels, so you always must use a low level technique like what is recommended in http://www.ibm.com/developerworks/java/library/j-benchmark1.html#dce
	2) similarly, random inlining decisions can occur at so many points in all but the smallest of benchmark code that you really must do multiple runs as described above
...
+++ need to add a statistical test that detects if the measurements are multimodal (i.e. clustered), since that would be very bad
	--multimode detection is definitely needed to detect the compilation planning variations described above
...
+++ it would be really slick if generated a plot of the results, since that would enable the user to tell at a glance if there is correlations, outliers, etc
...
* <b>If you need to perform different types of benchmarks, it is suboptimal to do them all in a single JVM session.</b>
* The issue is that the JVM may make optimization decisions (branch predictions, memory layout, etc)
* based on profiling information that is appropriate for the first benchmark, but is poor for subsequent ones.
...
public class Benchmark {
	...
	* <p>
	* Contract: is > 0.
	* Is 1 if <code>params.getManyExecutions()</code> returns <code>false</code>.
	*/
	...
	* <p>
	* Contract: is <code>null</code> if <code>params.getManyExecutions()</code> returns <code>false</code>,
	* else is non-<code>null</code> with length = <code>params.{@link Params#getNumberMeasurements getNumberMeasurements}</code>.
	*/
	...
	* <p>
	* Contract: is <code>null</code> if there are no issues.
	* Otherwise, is non null and has this format: a single short line that summaries the issue,
	* optionally followed by one or more tab indented lines which give the details.
	*/
	...
	* <p>
	* Contract: same as {@link #cleanIssues}.
	* In addition, is guaranteed to be null if <code>params.getManyExecutions()</code> returns <code>false</code>.
	*/
	...
	*/
	...
	/**
	* If {@link #params}.{@link Params#getManyExecutions getManyExecutions} returns true,
...
// +++ anything standard to do here?  need to talk to unix experts... see http://www.unix.com/showthread.php?p=302166530#post302166530
	...
	* This phase lasts for the time specified by {@link #params}.{@link Params#getWarmupTime getWarmupTime}.
	* <p>
	* @throws Exception (or some subclass) if <code>task</code> is a <code>Callable</code> and <code>task.call</code> throws it
	*/
	...
	* of their execution times to approximately equal {@link #params}.{@link Params#getExecutionTimeGoal getExecutionTimeGoal}.
	...
	*/
	protected void determineNumberExecutions() throws Exception {
		...
		Measurement mLower = null;
		for (long n = 1; ; ) {
			sendUserMsg("determining how many executions of task are required for executionTimeGoal to be met; trying n = " + n);
			Measurement m = measure(n);
			if (m.executionTime < params.getExecutionTimeGoal()) {	// n is too small
				sendUserMsg("n = " + n + " --> executionTime = " + m.executionTime + " which is < executionTimeGoal = " + params.getExecutionTimeGoal());
				mLower = m;
				n *= 2;	// double n and retry; note: if n overflows then measure will detect it
			}
			else if (m.executionTime == params.getExecutionTimeGoal()) {	// n is perfect
				sendUserMsg("n = " + n + " --> executionTime = " + m.executionTime + " which = executionTimeGoal, so will use " + n);
				numberExecutions = n;
				return;
			}
			else if (n == 1) {	// just a single execution exceeds executionTimeGoal, so must use 1
				sendUserMsg("n = 1 --> executionTime = " + m.executionTime + " which is > executionTimeGoal = " + params.getExecutionTimeGoal() + ", so will use 1");
				numberExecutions = 1;
				return;
			}
			else {	// n exceeds executionTimeGoal, so interpolate the result using mLower
				double nLower = n / 2.0;
				double timeLower = mLower.executionTime;	// mLower != null since only get to this else if have had at least one doubling of n
				
				double nUpper = n;
				double timeUpper = m.executionTime;
				
				double slope = (timeUpper - timeLower) / (nUpper - nLower);
				double offset = timeUpper - (slope * nUpper);
				
				double n1 = (1 - offset) / slope;
				n1 = Math.round(n1);
				n1 = Math.max(n1, 1);	// lower bound is 1
				
				numberExecutions = (long) n1;
				sendUserMsg("n = " + n + " --> executionTime = " + m.executionTime + " which is > executionTimeGoal = " + params.getExecutionTimeGoal() + "; using nLower = " + nLower + " and timeLower = " + timeLower + " can interpolate the desired result as " + n1);
				return;
			}
		}
	}
	// Note: the method above used to keep track of the JvmState, and not rely on a measurement exceeding 1 until it was stable.
	// Decided to drop this, since made the code more complicated, and because the procedure above is an approximation anyways.
	...
	*/
	protected void cleanJvmFinal() {
		...
		if (cleanFraction > params.getCleanFractionThreshold()) {
			cleanIssues =
				"execution times may be too small" + "\n"
				+ "\t" + "a final JVM cleanup took an extra " + (100 * cleanFraction) + "% of the total execution time" + "\n"
				+ "\t" + "this time should be included in the block measurements (perhaps mostly the final ones?)";
		}
	...
	*/
	protected void diagnoseOutliers() throws IllegalStateException {
		...
		if (lowExtremeList.size() + lowMildList.size() + highExtremeList.size() + highMildList.size() > 0) {
			...
			if (lowExtremeList.size() > 0) outlierIssues += "\n" + "\t" + lowExtremeList.size() + " are extreme outliers (on the low side): " + StringUtil.toString(lowExtremeList, ", ");
			if (lowMildList.size() > 0) outlierIssues += "\n" + "\t" + lowMildList.size() + " are mild outliers (on the low side): " + StringUtil.toString(lowMildList, ", ");
			if (highExtremeList.size() > 0) outlierIssues += "\n" + "\t" + highExtremeList.size() + " are extreme outliers (on the high side): " + StringUtil.toString(highExtremeList, ", ");
			if (highMildList.size() > 0) outlierIssues += "\n" + "\t" + highMildList.size() + " are mild outliers (on the high side): " + StringUtil.toString(highMildList, ", ");
		}
...
-->
	...
	*/
	protected void diagnoseSerialCorrelation() throws IllegalStateException {
		...
		int N = times.length;
		if (N < 50) {
			serialCorrelationIssues = "unclear if execution times have serial correlation or not: N = " + N + " < 50 is too small for autocorrelation tests";	// requiring N >= 50 is suggested by BOX-JENKINS on p. 33
			return;
		}
...
// How do these hypothesis tests relate to the large lag SE CI algorithm?
	...
	*/
	protected void calculateStats() throws IllegalStateException {
		sendUserMsg("calculating the block statistics (each measurement covers " + numberExecutions + " task executions)...");
		...
		sendUserMsg("calculating the action statistics (each measurement covers " + getNumberActionsPerMeasurement() + " action executions)...");
	...
	* <p>
	* Contract on result: same as {@link #cleanIssues}.
	* <p>
	...
	*/
	protected String diagnoseSd(double mean, double sd) {
		...
		if (!params.getEstimateNoiseFloor()) {
			return
				"standard deviation results have unknown validity" + "\n"
				+ "\t" + "the environmental noise test was skipped";
		}
		...
		if (sdFraction > params.getSdFractionThreshold()) {
			return
				"block standard deviation values may not reflect task's intrinsic variation" + "\n"
				+ "\t" + "environmental noise appears to explain " + (100 * sdFraction) + "% of the measured standard deviation";
		}
	...
	*/
	protected void checkState() throws IllegalStateException {
		...
		if (params.getManyExecutions()) {
			if (statsBlock == null) throw new IllegalStateException("params.getManyExecutions() returns true, but statsBlock == null");
			if (statsAction == null) throw new IllegalStateException("params.getManyExecutions() returns true, but statsAction == null");
		}
		else {
			if (numberExecutions != 1) throw new IllegalStateException("params.getManyExecutions() returns false, but numberExecutions = " + numberExecutions + " != 1");
			if (measurements != null) throw new IllegalStateException("params.getManyExecutions() returns false, but measurements != null");
			if (outlierIssues != null) throw new IllegalStateException("params.getManyExecutions() returns false, but outlierIssues != null");
			if (serialCorrelationIssues != null) throw new IllegalStateException("params.getManyExecutions() returns false, but serialCorrelationIssues != null");
		}
	}
	...
	*/
	...
	/** Returns the first <i>block</i> execution time (i.e. <code>{@link #timeExecFirst}</code>). */
	public double getFirstBlock() {
		return timeExecFirst;
	}
	
	/**
	* Returns the first <i>action</i> execution time.
	* Must be calcuated from the first block measurement using <code>{@link #getFirstBlock getFirstBlock} / {@link #params}.{@link Params#getNumberActions getNumberActions}</code>.
	* Therefore, the result is actually the mean of the first several action executions if getNumberActions > 1.
	*/
	public double getFirstAction() {
		return getFirstBlock() / params.getNumberActions();
	}
	
	/**
	* Alias for {@link #getFirstAction getFirstAction}.
	* This is a convenience method, since most users will typically only be interested in the action results.
	*/
	...
	*/
	@Override public String toString() {
		...
		if (params.getManyExecutions()) {
			...
			return
				first + "\n"
				+ issuesSummary();	// with current code below for issueSummary, this should always be empty for a single execution, but leave it here anyways in case issueSummary is changed or overridden
		}
	}
	
	/** Returns a summary of any issues that occured. */
	protected String issuesSummary() {
		StringBuilder sb = new StringBuilder();
		appendIssuesSummary(cleanIssues, sb);
		appendIssuesSummary(outlierIssues, sb);
		appendIssuesSummary(serialCorrelationIssues, sb);
		if (statsBlock != null) appendIssuesSummary(statsBlock.getSdIssues(), sb);
		if (statsAction != null) appendIssuesSummary(statsAction.getSdIssues(), sb);
		return sb.toString();
	}
	
	/** Helper method for {@link #issuesSummary issuesSummary}. */
	protected void appendIssuesSummary(String issues, StringBuilder sb) {
		if (issues == null) return;
		
		if (sb.length() == 0) sb.append("WARNING: ");
		else sb.append(", ");
		
		int index = issues.indexOf('\n');
		String lineFirst = (index >= 0) ? issues.substring(0, index) : issues;
		sb.append(lineFirst);
	}
	...
	public String toStringFull() {
		if (params.getManyExecutions()) {
			...
			sb.append("---------- action execution time statistics").append('\n');
			sb.append('\n');
			sb.append( toString() ).append('\n');
			
			sb.append('\n');
			sb.append("---------- calculation details").append('\n');
			sb.append('\n');
			if (getNumberActionsPerMeasurement() == 1) {
				sb.append("action results above directly taken from block results below because each block executed 1 action (there is no action/block distinction in this case)").append('\n');
			}
			else {
				sb.append("action results above calculated from block results below").append('\n');
				sb.append("number of actions per block is a = ").append( getNumberActionsPerMeasurement() ).append('\n');
				sb.append('\t').append("each block measured ").append( numberExecutions ).append(" task executions  (goal: make the execution time of the block >= ").append( params.getExecutionTimeGoal() ).append(" seconds)").append('\n');
				sb.append('\t').append("task internally performs m = ").append( params.getNumberActions() ).append(" actions").append('\n');
				sb.append("to calculate:").append('\n');
				sb.append('\t').append("action mean from block's, multiply block's by 1/a = 1/").append( getNumberActionsPerMeasurement() ).append(" = ").append( 1.0/getNumberActionsPerMeasurement() ).append('\n');
				sb.append('\t').append("action standard deviation from block's, multiply block's by 1/sqrt(a) = 1/sqrt(").append( getNumberActionsPerMeasurement() ).append(") = ").append( 1.0/Math.sqrt(getNumberActionsPerMeasurement()) ).append('\n');
				sb.append('\t').append("these formulas assume that the action execution times are independent and identically distributed (IID)").append('\n');
				sb.append("confidence interval (CI):").append('\n');
				sb.append('\t').append("reported as either +- deltas from the point estimate, or as a closed interval ([x, y])").append('\n');
				sb.append('\t').append("always has confidence level = ").append( params.getConfidenceLevel() ).append('\n');
			}
			
			sb.append('\n');
			sb.append("---------- block execution times (seconds)").append('\n');
			sb.append('\n');
			sb.append( StringUtil.toString(getTimes(), ", ") ).append('\n');
			
			if (getNumberActionsPerMeasurement() > 1) {
				sb.append('\n');
				sb.append("---------- block execution time statistics").append('\n');
				sb.append('\n');
				sb.append("first = ").append( getFirstBlock() ).append('\n');
				sb.append( getStatsBlock() ).append('\n');
			}
			
			sb.append( issuesFull() );
	...
	/** Returns full details of all issues that occured. */
	protected String issuesFull() {
		StringBuilder sb = new StringBuilder(1024);
		appendIssuesFull(cleanIssues, sb);
		appendIssuesFull(outlierIssues, sb);
		appendIssuesFull(serialCorrelationIssues, sb);
		appendIssuesFull(statsBlock.getSdIssues(), sb);
		appendIssuesFull(statsAction.getSdIssues(), sb);
		if (sb.length() > 0) sb.insert(0, "\n" + "---------- WARNING details" + "\n");
		return sb.toString();
	}
	
	/** Helper method for {@link #issueDetails issueDetails}. */
	protected void appendIssuesFull(String issues, StringBuilder sb) {
		if (issues == null) return;
...
*/
		...
		* This value is critical for extracting the action statistics from the block statistics (which come from what is actually measured);
		* see {@link Stats#forActions}.
		* <p>
...
// +++ would love to use the median too: is robust against outliers, plus could see if it differs radically from the mean as a skewness diagnostic.
...
// Similarly, I thought about reporting the best and worst times as well, but they have the same issue...
		...
		*/
		protected String diagnoseSdOfActions(double a) {	// CRITICAL: note that I am converting "a" from a long to a double in order to ensure that all math below is carried out in double precision, because I have seen errors with integer arithmetic spilling over into negative values when a gets large enough otherwise
			...
			else if (fractionVarOutlierMin < 0.10) msg = "slightly inflated";
			else if (fractionVarOutlierMin < 0.50) msg = "inflated";
			else msg = "highly inflated";
		...
		*/
		...
		@Override public String toString() {
			return
				...
				+ "standard deviation = " + FormatUtil.toEngineeringTime(getSd(), 3) + toStringCi(getSd(), getSdLower(), getSdUpper());
		}
	...
	*/
	...
	public static class UnitTest {
		...
		*/
		...
		/**
		* Results on 2011-07-10 (2.5 GHz Xeon E5420 desktop, jdk 1.6.0_25 server jvm):
		* <pre><code>
		...
		*/
		@Test public void test_Benchmark_single() throws Exception {	// do NOT name this benchmark_Thread_sleep as per the other methods below, instead start it's name with test_ so that it will be executed by my normal JUnit automatic suite run
			final int sleepTime = 2 * 1000;
			Callable<Long> task = new Callable<Long>() {
				public Long call() throws InterruptedException {
					long t1 = System.nanoTime();
					Thread.sleep(sleepTime);
					long t2 = System.nanoTime();
					return t2 - t1;	// needed to prevent DCE since this is a Callable
				}
			};
			
			Benchmark benchmark = new Benchmark(task, false);
			System.out.println("Thread.sleep: " + benchmark.toString());
			Assert.assertEquals(sleepTime * 1e-3, benchmark.getFirst(), 10e-3);
			System.out.println("Benchmark correctly measured a known execution time task using a single execution measurement.");
		}
		...
		*/
		@Test public void test_Benchmark_multiple() throws Exception {	// do NOT name this benchmark_Thread_sleep as per the other methods below, instead start it's name with test_ so that it will be executed by my normal JUnit automatic suite run
			...
			System.out.println("Benchmark correctly measured a known execution time task using multiple execution measurements.");
		}
