	...
	*/
	public static long makeSeed() {
		...
		try {
			...
			Logger.getAnonymousLogger().logp(Level.FINER, "RandomUtil", "makeSeed", "Good: makeSeedSecure worked, so makeSeed will use a cryptographically strong seed as part of its initialization");
		}
		catch (Throwable t) {
		    Logger.getAnonymousLogger().logp(Level.INFO, "RandomUtil", "makeSeed", "Note: makeSeedSecure generated the following Throwable, so makeSeed will fall back on another algorithm", t);
