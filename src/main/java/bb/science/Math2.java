...
public class Math2 {
	...
	// -------------------- fundamental math: compare, equals, modulo, sign, hasSameSign --------------------
	...
	* this method returns true in that case, whereas <code>d1 == d2</code> returns false because NaN always returns false when used in tests.
	*/
	...
	*/
	...
	// -------------------- combinatorics: binomialCoefficient --------------------
	
	/**
	* Computes the <a href="http://en.wikipedia.org/wiki/Binomial_coefficient">binary coefficient</a> C(n, k).
	* <p>
	* The implementation here is a
	* <a href="http://en.wikipedia.org/wiki/Binomial_coefficient#Binomial_coefficient_in_programming_languages">direct implementation of the multiplicative formula</a>.
	* <p>
	* @throws IllegalArgumentException if n < 0, k < 0, n < k
	* @throws IllegalStateException if a valid int value cannot be returned
	*/
	public static int binomialCoefficient(int n, int k) throws IllegalArgumentException, IllegalStateException {
// +++ should this method take and return long instead of int?
// +++ should it internally use BigDecimal in order to guarantee exact results?
		Check.arg().notNegative(n);
		Check.arg().notNegative(k);
		Check.arg().isTrue(n >= k);
		
		if (k > n/2) k = n - k; // take advantage of symmetry to compute fewer terms in the loop below
		
		double accum = 1;
		for (int i = 1; i <= k; i++) {
			accum *= (n - k + i) / ((double) i);
		}
		Check.state().normalPositive(accum);	// CRITICAL: must do this check before call round below, since Java's round method is defective (fails to detect NaN or infinity)
		long round = Math.round(accum); // avoid rounding error
		if (round > Integer.MAX_VALUE) throw new IllegalStateException("the result = " + round + " is too large to be represented as an int");
		return (int) round;
	}
...
return Double.parseDouble("1E" + power);
	...
	*/
	...
	// -------------------- statistics: gaussianPdf, gaussianCdf, gaussianFit, gaussianAndersonDarling, gaussianKolmogorovSmirnov --------------------
...
*/
	...
	*/
	...
	// -------------------- autocoXXX statistics calculations --------------------
...
// +++ there is a faster linear algorithm for medians if the numbers are not sorted: http://en.wikipedia.org/wiki/Selection_algorithm
	...
	* <p>
	* Statistically speaking, if numbers is the population, then the result is the population's standard deviation.
	* But if numbers is just a sample from the population, then the result is merely an estimate of the standard deviation.
	* <p>
	* This method simply returns the square root of {@link #variance(double[], double, boolean) variance}(numbers, mean, biased).
	* <i>Therefore, the mean and biased parameters must have the exact meanings expected by variance</i> (see its javadocs).
	* In particular, the biased parameter will control whether or not the <i>variance</i> estimate is biased.
	* <i>It does not control the bias of the standard deviation estimate returned by this method.</i>
	* <p>
	...
	*	<a href="https://secure.wikimedia.org/wikipedia/en/wiki/Standard_deviation#With_standard_deviation_of_the_sample">Reference</a>
	* </blockquote>
	...
	* But if numbers is just a sample from the population, then the result is merely an estimate of the population's variance.
	* <p>
...
but what is the OPTIMAL sd est?
	--Note that for estimating the variance, as opposed to the sd, it looks like the divide by n + 1 formula is not only lower in MSE than the the divide by n - 1 formula but is even lower in MSE than the divide by n formula
...
0) what are the optimal estimators for the common stats?
	--known results:
		a) the / n mean estimator is the optimal UNBIASED one given a Gaussian distribution, but is known to NOT be optimal for auniform distribution
		b) the / (n - 1) variance estimator is NOT the minimal MSE one; instead, you always do better (lowering MSE, at the cost of being biased) by scaling it down a bit, for example, / (n + 1) is the minimal MSE one given a Gaussian distribution
	Source for the above:
		https://secure.wikimedia.org/wikipedia/en/wiki/Mean_squared_error#Examples
...
if instead of +- the SAME constant from the mean, you did + of one amount and - of a different amount?
...
3) if you have multiple estimators, can you combine them somehow to, say, get narrower confidence intervals?
...
-->
	...
	* <p>
	* Contract: the result is always >= 0 (with positive infinity allowed), and is never NaN.
	* <p>
	* @throws IllegalArgumentException if numbers == null; numbers.length == 0; mean or any element of numbers is NaN
...
-->
		*/
		...
		@Test public void test_binomialCoefficient() {
				// Pascal's triangle:
					// row 0:
			Assert.assertTrue( binomialCoefficient(0, 0) == 1 );
					// row 1:
			Assert.assertTrue( binomialCoefficient(1, 0) == 1 );
			Assert.assertTrue( binomialCoefficient(1, 1) == 1 );
					// row 2:
			Assert.assertTrue( binomialCoefficient(2, 0) == 1 );
			Assert.assertTrue( binomialCoefficient(2, 1) == 2 );
			Assert.assertTrue( binomialCoefficient(2, 2) == 1 );
					// row 3:
			Assert.assertTrue( binomialCoefficient(3, 0) == 1 );
			Assert.assertTrue( binomialCoefficient(3, 1) == 3 );
			Assert.assertTrue( binomialCoefficient(3, 2) == 3 );
			Assert.assertTrue( binomialCoefficient(3, 3) == 1 );
					// row 4:
			Assert.assertTrue( binomialCoefficient(4, 0) == 1 );
			Assert.assertTrue( binomialCoefficient(4, 1) == 4 );
			Assert.assertTrue( binomialCoefficient(4, 2) == 6 );
			Assert.assertTrue( binomialCoefficient(4, 3) == 4 );
			Assert.assertTrue( binomialCoefficient(4, 4) == 1 );
		}
