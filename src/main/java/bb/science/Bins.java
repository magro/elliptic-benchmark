...
--2 general approaches to bin intervals are:
	1) equal widths
		--this is the approach used in the Intervals inner class below
		--is natural to interpret
...
* The sole restriction placed on the values is that they must be {@link Check#normal normal}.
...
* <h4>Intervals</h4>
* This class guarantees that the set of bin intervals it generates will always cover every value supplied to this class.
...
* <p>
* This class <i>usually</i> uses the following closed-open bin interval convention:
...
* <p>
* The <i>sole exception</i> is the rightmost (largest) interval: it is a fully closed interval of the form <code>[x0, x1<b>]</b></code>.
...
* For example, suppose the user is generating histograms of percents, and they want 5 bins to cover the range <code>[0, 100]</code>.
* Then with the interval scheme just described, the 5 intervals in this case will be
* <code>[0, 20), [20, 40), [40, 60), [60, 80), [80, 100<b>]</b></code>.
* <p>
* So, this class always uses a finite set of contiguous intervals
* which can be written as <code>[x0, x1), [x1, x2), [x2, x3), ..., [xN-1, xN]</code>
* where <code>N</code> is the total number of intervals and the numbers are in ascending order.
...
public class Bins {
	...
	* <p>
	* <b>Warning:</b> to achieve top performance, the implemenation here returns the field, not a copy.
	* <i>This breaks encapsultation: mutating the result invalidates this instance.</i>
	*/
	...
	* <p>
	* <i>Note:</i> to spare users which do not care about this result the costs of computing and storing it,
	* the implemenation here calculates a new result each time.
	* Users which need to reuse the result multiple times must cache it to achieve top performance.
	*/
...
// that other Java graphing packages could use, assuming that they take that format
	...
	* <p>
	* The left (start) boundary of the leftmost (first) interval is given by {@link #begin}.
	* The right (end) boundary of the rightmost (last) interval is given by {@link #end}.
...
/*
+++ finish this and test it too
		private static Intervals make(double[] values, double offsetMid, int number) throws IllegalArgumentException, IllegalStateException {
			// values checked by minMax below
			Check.arg().normal(offsetMid);
...
*/
		...
		*  <li>the left (starting) boundary of the leftmost (first) interval equals the minimum element of values</li>
		*  <li>the right (ending) boundary of the rightmost (last) interval equals the maximum element of values</li>
		* </ol>
		* <p>
		* This method finds the min and max elements of values,
